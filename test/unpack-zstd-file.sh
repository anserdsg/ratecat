...
temp_zstd_file=$(mktemp)

openssl enc -d -aes-256-cbc -salt -pbkdf2 -in $temp_file -out $temp_zstd_file -k $password

zstd -d $temp_zstd_file -o $output_file

rm -f $temp_file $temp_zstd_file
